...
async function loadSettings(): Promise<void> {
  for (const platform of PLATFORMS) {
    try {
      ...
      const checkbox = document.getElementById(`sync_${platform}`) as HTMLInputElement | null;
      if (!checkbox) {
        console.warn(`[AIChatFolders] Checkbox not found for platform: ${platform}`);
        continue;
      }
      checkbox.checked = settings.syncNativeChanges;

      const handleCheckboxChange = async () => {
        try {
          const updated = await FolderManager.updateDomainSettings(
            { syncNativeChanges: checkbox.checked },
            platform,
          );
          console.log(`[AIChatFolders] Saved ${platform} sync setting:`, updated);
          showToast('saved');
        } catch (error) {
